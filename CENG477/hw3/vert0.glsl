...
uniform mat4 SRMat;
uniform mat4 SRMatInv;
uniform mat4 modelingMat;
uniform mat4 perspectiveMat;

// our declerations
uniform float objectX;
uniform float objectY;
...
void main(void)
{
	...
	vec4 p = SRMat * vec4(inVertex, 1); // translate to world coordinates
	vec3 Lorg = lightPos - vec3(p);
	vec3 L = normalize(Lorg);
	vec3 V = normalize(eyePos - vec3(p));
	vec3 H = normalize(L + V);
	vec3 N = vec3(SRMatInv * vec4(inNormal, 0)); // provided by the programmer
	N = normalize(N);
	float NdotL = dot(N, L);
	float NdotH = dot(N, H);
